...
const copyDatabase = async (pluginsPath: string) => {
  ...
  if (!existsSync(databasePath)) {
    console.log(`⛔️ Database not found in 'src/database' directory. "${databasePath}"`);
    process.exit(1);
  }
  ...
  if (!existsSync(join(destinationPath, "client.ts"))) {
    await cp(join(databasePath, "client.ts"), join(destinationPath, "client.ts"));
  }
};

export const prepareFiles = async ({ pluginsPath }: { pluginsPath: string }) => {
